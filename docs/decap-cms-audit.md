...
## October 2025 Widget & Media Enhancements
- **Reusable CTA object** — Hero, showcase cards, banners, and legacy image blocks now reference a single CTA widget with URL validation, optional new-tab behaviour, aria-label overrides, and style tokens. Editors can reuse the same pattern everywhere instead of juggling slightly different link groups.【F:admin/config.yml†L76-L112】【F:admin/config.yml†L486-L508】【F:admin/config.yml†L612-L636】【F:admin/config.yml†L758-L770】
- **Minimal Markdown toolbar** — Long-form fields only expose headings, emphasis, links, and list formatting so the editor focuses on the essentials without digging through unused options.【F:admin/config.yml†L373-L408】【F:admin/config.yml†L748-L758】
- **Shared image anchors** — Section, gallery, carousel, and timeline uploads all inherit the same image/alt configuration, which enforces consistent hints and enables the new media library to surface shared metadata.【F:admin/config.yml†L391-L408】【F:admin/config.yml†L187-L205】【F:admin/config.yml†L500-L516】【F:admin/config.yml†L538-L556】【F:admin/config.yml†L704-L731】
- **Cloudinary-backed media library** — Decap now loads the Cloudinary media library (set `CLOUDINARY_CLOUD_NAME` and `CLOUDINARY_API_KEY` in Netlify) and introduces a `Media / Images` collection that requires alt text plus at least one descriptive tag per asset.【F:admin/config.yml†L10-L27】【F:admin/config.yml†L745-L769】

### When to use the new widgets
- Add CTAs with the shared widget whenever a block needs a button or link. The summary row will show “Label → URL,” making it easy to spot missing links before publishing.【F:admin/config.yml†L76-L112】【F:admin/config.yml†L486-L508】【F:admin/config.yml†L612-L636】
- Keep long-form copy readable by leaving Markdown fields on the minimal toolbar; if you need additional formatting, flag it to the CMS maintainers instead of toggling the Markdown view manually.【F:admin/config.yml†L373-L408】【F:admin/config.yml†L748-L758】
- Upload hero, gallery, and carousel imagery via the Cloudinary picker, then catalogue reusable assets in `Media / Images` so alt text and tagging stay consistent across pages.【F:admin/config.yml†L187-L205】【F:admin/config.yml†L500-L556】【F:admin/config.yml†L745-L769】
