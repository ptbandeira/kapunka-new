...
## 2025-10-11 — Switched Page Builder to JSON index
...
## 2025-10-11 — Trimmed unsupported Page Builder metadata
- **What changed**: Removed the `label_singular` and `description` props from the JSON-backed `pages` collection in `admin/config-modules/main.yaml`, regenerated `admin/config.yml`.
- **Impact & follow-up**: Ensures Decap treats the Page Builder entry as a valid single-file collection so the CMS no longer rejects the configuration. Confirm previews against the JSON index remain available before proceeding with the Markdown migration cleanup.
- **References**: Pending PR
